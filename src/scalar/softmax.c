...
static float max__scalar(size_t n, const float v[C99(restrict static) n]) {
	float max_v = *v++;
	while (--n) {
		max_v = maxf(max_v, *v++);
	}
...
static float sum_exp_minus_c__scalar(size_t n, const float v[C99(restrict static) n], float c) {
	float sum = 0.0f;
	do {
		sum += expf(*v++ - c);
	} while (--n);
...
static void scaled_exp_minus_c__scalar(size_t n, const float x[C99(restrict static) n], float y[C99(restrict static) n], float scale, float c) {
	do {
		*y++ = scale * expf(*x++ - c);
	} while (--n);
}
